...
void MainWindow::deleteNode()
{
    if(scene->items().count() != 0)
    {
        if (scene->selectedItems().count() > 0)
        {
            ...
            QList <QGraphicsItem *> linesList = scene->items();
            for(int x = 0; x < itemsList.count(); x++)
            {
                ...
                {
                    for (x=0 ; x<linesList.count(); x++)
                    {
                        if(linesList[x] == line)
                        {
                            if (line->line().p1() == itemsList[x]->pos() || line->line().p2() == itemsList[x]->pos())
                            {
                                delete linesList[x];
                            }
...
{
    ConnectNodes *connectLine = new ConnectNodes(scene);
    connectLine->addLine();
}
